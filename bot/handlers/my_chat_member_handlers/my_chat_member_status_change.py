...
from db import Group, RPSLSStats, User, async_session
from settings import logger


async def my_chat_member_status_change_handler(message: ChatMemberUpdated):
    logger.warning(
        f"User: {message.from_user.id} changed bot status in chat: {message.chat.id}"
    )
    ...
    async with async_session() as session:
        async with session.begin():
            ...
            if not registered_group:
                logger.debug(
                    f"Group: {message.chat.id} is not in database. Adding. Admin: {message.from_user.id}"
                )
                ...
                logger.info(
                    f"Group: {message.chat.id} is already in database, setting inactive=False"
                )
                await session.execute(
                    update(Group)
                    .where(Group.id == registered_group.id)
                    .values(inactive=False)
                )

            else:
                logger.info(
                    f"Group: {message.chat.id} expelled bot. Setting inactive=True"
                )
                await session.execute(
                    update(Group)
                    .where(Group.id == registered_group.id)

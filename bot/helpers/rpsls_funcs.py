from messages import BOTH_CHOSE_MESSAGE, HAVE_WON_MESSAGE, TIE_MESSAGE
...
def eval_gif_path(win_fig, loose_fig) -> str:
    if win_fig:
        gif_path = (
            f"{STATIC_PATH}{win_fig.class_name_lower()}/"
            f"{win_fig.class_name_lower()}-{loose_fig.class_name_lower()}.gif"
        )
        return gif_path
    gif_path = (
        f"{STATIC_PATH}{loose_fig.__class__.__name__.lower()}/"
